...
from RishavMusic import YouTube, app
from RishavMusic.core.call import Rishav
from RishavMusic.misc import SUDOERS, db
from RishavMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from RishavMusic.utils.decorators.language import languageCB
from RishavMusic.utils.formatters import seconds_to_min
from RishavMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from RishavMusic.utils.stream.autoclear import auto_clean
from RishavMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Rishav.pause_stream(chat_id)
        ...
        await Rishav.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Rishav.stop_stream(chat_id)
        ...
        await Rishav.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Rishav.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Rishav.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Rishav.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Rishav.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
